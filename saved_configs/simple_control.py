...
# [0,255]
max_id = 255
total_object_types = 8 # including 0
margin = int(max_id/total_object_types)

BG = margin*0
CHAIR = margin*1  # 31
ROUND_TABLE = margin*2
COMMERCIAL_PLANE = margin*3
CLASSIC_SOFA = margin*4
MODERN_SOFA = margin*5
COMMODE = margin*6
AIRLAB_GATE = margin*7
TANK = margin*8
...
class SimpleTerminalController:
    ...
    def confirm_connection(self):
        a = self.client.confirmConnection()
        print(a)
        #self.client.enableApiControl(True) # UNCOMMENT THIS IF USING DRONE
    ...
    def setup_segmentation_colors(self):
        ...
        self.change_color("CHAIR", CHAIR)
        self.change_color("ROUND_TABLE", ROUND_TABLE)
        self.change_color("COMMERCIAL_PLANE", COMMERCIAL_PLANE)
        self.change_color("CLASSIC_SOFA", CLASSIC_SOFA)
        self.change_color("MODERN_SOFA", MODERN_SOFA)
        self.change_color("COMMODE", COMMODE)
        self.change_color("AIRLAB_GATE", AIRLAB_GATE)
        self.change_color("TANK", TANK)
    ...
    def move_to_position(self, args: list):
        ...
        print("Move args:", float(args[1]), float(
            args[2]), float(args[3]), float(args[4]))
    ...
    def move_on_path(self, args: list):
        ...
        try:
            result = self.client.moveOnPathAsync(path, float(args[-1]), TIMEOUT,
                                                 airsim.DrivetrainType.ForwardOnly, airsim.YawMode(
                                                     False, 0),
                                                 20,
                                                 1).join()
        except:
